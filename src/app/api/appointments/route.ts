/**
 * @fileoverview API route for managing appointments
 * @module api/appointments
 * @author Santiago Prada
 * 
 * This module provides endpoints for creating appointments with authentication,
 * Google Calendar integration, and database persistence.
 * 
...
/**
 * Handles POST request for creating appointments with comprehensive validation and integration.
 * 
 ...
 * 3. **Entity Verification**: Ensures doctor, patient, and service exist and belong to the same organization
...
async function handlePostRequest(
  ...
  try {
    ...
    // Verify doctor belongs to the same organization
    if (doctor.user?.organizationId !== userOrganizationId) {
      return createErrorResponse(
        API_ERRORS.FORBIDDEN,
        'El doctor no pertenece a su organización',
        HTTP_STATUS.FORBIDDEN
      );
    }
    ...
    if (patient.organizationId !== userOrganizationId) {
      return createErrorResponse(
        API_ERRORS.FORBIDDEN,
    ...
    if (medicalService.organizationId !== userOrganizationId) {
      return createErrorResponse(
        API_ERRORS.FORBIDDEN,
        'El servicio médico no pertenece a su organización',
        HTTP_STATUS.FORBIDDEN
      );
    }
    ...
    const appointmentDuration = durationMinutes || medicalService.durationMinutes;
    const endDateTime = startDateTime.plus({ minutes: appointmentDuration });
    const calculatedEndTime = endTime || endDateTime.toFormat('HH:mm');
    ...
    const newAppointment = await db.insert(appointments).values({
      ...
      serviceId: medicalService.id,
      organizationId: userOrganizationId,
      appointmentDate: startDateTime.toJSDate(),
...
/**
 * POST endpoint for creating appointments with comprehensive validation and integration.
 * 
 ...
 *   "details": "Campos requeridos faltantes: doctorId, patientId"
 * }
 * ```
 * 
